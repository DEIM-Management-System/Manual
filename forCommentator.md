# DEIM 座長・コメンテータ用マニュアル

## ハラスメントについて
* 発表者（特に学生さん）に対する対応において，不快な思いや不安を感じさせないように，改めて細心のご注意をお願いします．
...
* **せっかく入念に事前準備していただいても，当日に会場からの質疑が盛り上がって準備が全て無駄に終わる可能性もあります．** 特にコメンテータの皆様は，当日に全く出番が無くなることも起こりえます．ただ，これは学会としては望ましい状況ですので，申し訳ありませんが会場の質疑を優先してあげていただければと思います． 
	...
	* 座長・コメンテータの皆様は，発表時間内に伝えられなかったコメントは，『**掲示板**』において発表者へお伝えすることができます（**ぜひ，伝えてあげてください**）．DEIMホームページからDEIMポータルに入りますと，各セッションの「発表会場」ボタンと「質疑会場」ボタンに並んでに「掲示板」ボタンがあります．コメントと共にスレッドを作成してください．スレッド作成は掲示板ページの右上にございます．スレッドを建てる際に発表へ『メンション』する事ができます．コメントをする対象発表をドロップダウンメニューからお選びください．メンションされた発表者のポータルサイトに，コメントがあった旨の通知が表示されます．
	

## 当日のオペレーション
1. 各セッションのWebex会議室は遅くとも開始時刻の10分前までには準備されます．
1. Webexに最初に入る際に，名前の入力を要求されるので，**「A13座長：氏名_所属」「F31コメ：氏名_所属」** などと名前に設定して，参加者に分かるようにしてください．
	* 大学教員の方は **「A13座長：★横山昌平_東京都立大学」** のように、氏名の前に★印をつけてください．
	* 2回目以降は，Webex会議から退出している状態で表示名を変更する必要があります．[表示される名前を後から変更する方法](#cisco-webex-meetingsにおけるユーザ名の変更の仕方)
2. セッション開始10分前に，ブラウザで[DEIM2022プログラム](https://cms.dbsj.org/deim2022/program/)を開き，「発表会場」というボタンを押し，[Webex Meeting](https://mediafiles.webex.com/ja/downloads.html)のセッションの会議室に入ってください．
	...
	* その他，**時間までに入れそうにない場合は，当日までに共有される電話番号（緊急連絡先）へ連絡をお願いします．**
...
4. すべての発表が終了したら，本セッションの発表の終了を宣言し，**この後にoVice会場で質疑が続くことをアナウンスする．**
	* oViceのURLは学生アルバイトがチャット欄に流します．
	* 山下記念賞の投票を呼び掛ける（投票URLは学生アルバイトの方がチャット欄に流します）．
	* その他，依頼があったアナウンスを伝える． 
5. チャット欄に流されたoViceのURLをクリックする．oViceがブラウザで開かれる．
    * **oViceへアクセスするブラウザは必ずChromeを使用してください．**
    * oViceでの質疑会場には，ブラウザで[DEIM2022プログラム](https://cms.dbsj.org/deim2022/program/)を開き，そこの「質疑会場」ボタンからも入ることが出来ます．
    * oViceを初めて使う際に名前の入力を求められます．こちらも **「A13座長：氏名_所属」「F31コメ：氏名_所属」という形式** でご登録ください．一旦設定した名前を変更するには，画面上の自分のアイコンをクリックすれば設定画面が表示されます．
    	* 大学教員の方は **「A13座長：★横山昌平_東京都立大学」** のように、氏名の前に★印をつけてください．
...
8. oVice上では，各発表者がブースに分かれて待っているので，順に回って，質問やコメントを伝えて回る．
    * 各質疑は並行して行われる形になります（一般的なポスター発表の形式です）．
    * 座長は順番にブースを回り，質疑が白熱していたら割込まずに見守り，質疑が出ていなかったらサポートをお願いします．
        * 盛り上がっていそうならば，空いている部屋から順に回るのが良いと思います．
    * 時間配分を考えて，なるべく全ての発表者に話しかけるようにお願いします．
    * 特に学生の発表は学生プレゼンテーション賞の対象ですので，座長の方は全ての学生発表者のブースに一度は入るようにお願いします．
    	* 質疑での議論もなく選外とされると，公平ではないと考える学生さんがいるかもしれません．
    	* 座長さんは全ての学生さんのブースに一度は立ち寄ってもらいたいと思います．質問しなくても，他の方が質問して盛り上がっているのを聞いているだけでも結構です．
    * 座長とコメンテータは一緒に回っても別々に回っても構いません．その場でお二人で相談して決めていただければと思います．
    * 特定の学生のブースに質問者が長居している場合は様子を伺ってみてください．**ハラスメントはオンラインでも発生します．**
    	* ハラスメント行為を見かけた場合は ahp@dbsj.org へご連絡ください．
9. 質問をするには発表者のブースへ行き，「oVice会議室」へ入る必要があります．四角い枠の中をクリックすることで入れます．
...
11. 座長は，学生プレゼンテーション賞の選考結果を提出する．
	...
	* 入力欄は評価シートの前半部分にあります．一時保存で提出できます（学生プレゼンテーション賞は各日のクロージングで表彰するので，非常にタイトです）
		* https://cms.dbsj.org/deim2022/program/chair.php
...
### インタラクティブ発表について
* 発表の中には，同じ日の最後に開催されるインタラクティブセッションでも発表を持つものがあります．
* インタラクティブセッションにも可能な限りご参加いただき，活発な質疑と，良い発表には投票（イイね）をお願いします．
	* 投票が最優秀インタラクティブ賞・優秀インタラクティブ賞を決めます．
	* 座長・コメンテータの投票は票の重みを増すことを考えています．
* インタラクティブセッションへの参加の仕方は，[一般参加者（聴講者）用マニュアル](forAudience.md)をご参照ください．

### その他注意事項
* 各セッションに，学生アルバイトによるセッション管理者（従来の会場係に相当）が張り付いているので，マイクの一斉ミュート等，必要に応じて依頼してください．
	* 発表者の音声が聞こえなくなったなど**大きなトラブルの場合は，学生アルバイトに「slackで緊急アラートを上げてください」と伝えれば，プラグラム委員長か副委員長が駆け付けます．**
* 自身が通話もチャットもできなくなったというトラブルの場合は，当日までに共有される電話番号（緊急連絡先）へ連絡をお願いします．
* **NO SHOWの定義について**
    * DEIM 2022の口頭発表は，「Webexにおいてプレゼンをした ＋ oViceで質疑をした」の2点が揃うことで「発表をした」と見做します．
    	* 動画によるプレゼンも許可します．ただし，推奨はしませんのでこの事実は積極的には公開しません．また，動画を再生するのは共著者の責任で，座長や運営が代行することはありません．
    	* 質疑が全く行われなかった場合はNO SHOWとして報告をお願いします．
    * DEIM 2022のインタラクティブ発表は，「時間の一部であってもoViceで発表した」ことで「発表をした」と見做します．
    	* 動画による発表（説明）も許可します．ただ，動画を再生するのは共著者の責任で，座長や運営が代行することはありません．また，質疑を共著者が受ける必要があります．
    	* 座長が見て回ったときにポスターブースに不在の場合は，時間を変えてもう一度程度訪れて，それでも居なければNO SHOWとして取り扱っていただいて構いません．
    * 発表者は，第一著者でも共著者でも同様に扱います．
    	* 学生の代わりに先生が発表するのも問題ありません．ただし，学生へ贈られる表彰の対象ではなくなります．
    	* 録画による学生の発表動画が秀逸で，ただし質疑は先生によって行われた場合は，それを学生へ贈られる表彰の対象とするかどうかは，最終的な判断は座長に委ねられます．
...
* コメンテータの皆様は，以下の提出物があります．
	1. コメンテータ評価シート（会議終了後一週間以内までに）
		* https://cms.dbsj.org/deim2022/program/commentator.php
		* 技術報告の発表にも「評価シート」がありますが，こちらは評価対象ではありませんので，記入の必要はございません．
* 各セッションには『掲示板』が用意されています
	* DEIMポータルに入りますと，「発表会場」ボタン・「質疑会場」ボタンと並んで「掲示板」ボタンがあります．
...
## 緊急時の対応について
### 緊急時対応1
予定されたWebex会議室へアクセスできない場合は，ポータルトップの案内をご確認ください．トラブルが起きてWebex会議室が立ち上がらなかった場合など，予備のWebex会議室へ移動になることがあります（アクセスURLが変更になります）．その場合はポータルトップへ緊急アナウンスを出します．

### 緊急時対応2
もしポータルをホストしているサーバがダウンするなどの致命的な障害が発生した場合は，会議の各セッションのアクセスURLを纏めたドキュメントを全参加者へメールするか，アクセスURLが晒されるのを承知でDEIMホームページ上で公開するか，という対応を用意しています（Webサーバは別ホストです）．また，座長・コメンテータの皆様には私から評価シートをメールでお送りし，メールベースでの提出をお願いすることになります．

### 緊急時対応3
当日の緊急連絡先は下記のとおりです．
* **1分を争う緊急事態**：プログラム委員長か副委員長の携帯番号をメールでお送りします．そちらへお電話ください．
* **30分以上の猶予はある件**：slack の「#座長コメンテータ」チャンネルへお書きいただくか，deim2022-inquiry@googlegroups.com へメールしてください（プログラム委員長・副委員長にも届きます）
* **会議室へ入った後の緊急連絡**：会場係の学生アルバイトの方へ依頼していただければ，学生アルバイトがslackのemergencyチャネルへ書き込み，プログラム委員長や副委員長をその会議室へ呼び出せます．
